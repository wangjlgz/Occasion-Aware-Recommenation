...
def multihead_attention_residual(queries, 
                        keys, 
                        num_units=None, 
                        num_heads=8, 
    ...
    def __call__(self, inputs, mask):
        ...
        for i in range(self.num_blocks):
            with tf.variable_scope("num_blocks_%d" % i):
                # Self-attention
                inputs = multihead_attention_residual(queries=normalize(inputs),
                                               keys=inputs,
...
def multihead_attention_original(queries, 
                        keys, 
                        values,
                        num_units=None, 
                        num_heads=8, 
                        dropout_rate=1.0,
                        causality=False,
                        scope="multihead_attention_original", 
                        reuse=None,
                        with_print_weights = False):
    '''
    Applies multihead attention.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        print_weight = tf.nn.softmax(outputs)
    ...
    if with_print_weights:
        return outputs, print_weight
    else:
        return outputs
